...
public class Utils 
{
	...
	public static void registrarServicio(Agent agent, String nombreServicio, String tipoServicio) {
		DFAgentDescription descripcionAgente = new DFAgentDescription(); 
		descripcionAgente.setName(agent.getAID()); 
		...
		descripcionServicio.addOntologies("ontologia"); 
		descripcionServicio.addLanguages(new SLCodec().getName());
		
		descripcionAgente.addServices(descripcionServicio); 
		
		try { 
			DFService.register(agent, descripcionAgente); 
		} catch (FIPAException ex) {
			System.err.println("Agente " + agent.getLocalName() + ": " + ex.getMessage()); 
		}
	}
}
